...
from core.utils import UNITY_DEVICE_ID, ARDUINO_DEVICE_NAME, UNITY_DEVICE_NAME, LIVEAMP_DEVICE_NAME, LIVEAMP_DEVICE_NAME_BRAINVISION, EEG_NAME
...
def npz_eeg_dataloader(datapack_name):
    # Rebuild a clean mne.io.RawArray from a plain NumPy .npz container.
    z = np.load(datapack_name, allow_pickle=False)
    info = mne.create_info(ch_names=[str(c) for c in z['ch_names']],
                           sfreq=float(z['sfreq']), ch_types='eeg')
    raw = mne.io.RawArray(z['data'].astype(np.float64), info, verbose=False)
    raw.set_annotations(mne.Annotations(z['ann_onset'], z['ann_duration'],
                                        [str(d) for d in z['ann_description']]))
    return raw
...
def make_individual_data(subject_name: str, exclude_device_data: List[str] = []) -> dict:
    ...
    for datapack_name in get_datapack_names(subject_name):
        ...
        elif EEG_NAME in datapack_name and EEG_NAME not in exclude_device_data:
            individual_data['eeg_clean'] = npz_eeg_dataloader(datapack_name)
