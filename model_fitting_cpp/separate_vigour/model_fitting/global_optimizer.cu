...
typedef std::unordered_map<std::string, 
                       std::vector<
                            std::tuple<float, // pain intensity
                                        ...
                                        uint32_t*>>*> FrameBlocksBySubjects;
...
#if !(DEBUG_SINGLE_TEST || TEST_AGAINST_PYTHON)
...
void fitAllSubjects(std::unordered_map<std::string, float*>& results, 
                    FrameBlocksBySubjects frame_blocks,
                    std::vector<int> slice_type,
                    std::unordered_map<std::string, std::vector<std::vector<float>>> slices_by_subjects)
{
    for (auto it = frame_blocks.begin(); it != frame_blocks.end(); ++it)
    {
        ...
        uint64_t grid_size = generateGrid(num_ticks, parameter_ticks, slice_type, slices_by_subjects[subject]);
...
std::unordered_map<std::string, std::vector<std::vector<float>>> generateSliceData(const FrameBlocksBySubjects* frame_blocks, const std::unordered_map<std::string, float*>* results=nullptr)
{
    std::unordered_map<std::string, std::vector<std::vector<float>>> slices_by_subjects;
    if (results == nullptr)
    {
        // First level fitting params goes here:
        #ifdef TEST_AGAINST_PYTHON
        ...
        for (auto it = frame_blocks->begin(); it != frame_blocks->end(); ++it)
        {
            ...
            slices_by_subjects[subject] = slices;
        }
    }
    else
    {
        ...
        for (auto it = results->begin(); it != results->end(); ++it)
        {
            const auto& pair = *it;
            auto subject = pair.first;
            auto fitting_result = pair.second;

            #ifdef TEST_AGAINST_PYTHON
            std::vector<std::vector<float>> slices = {
                {fitting_result[0] - 1, fitting_result[0]},
                {fitting_result[1] - 0.1f, fitting_result[1], 0.1},
                {max(fitting_result[2] - 1, 1.0f), min(fitting_result[2], 100.0f), 1},
                {max(fitting_result[3] - 1, 1.0f), min(fitting_result[3], 100.0f), 1},
                {max(fitting_result[4] - 1, 1.0f), min(fitting_result[4], 100.0f), 1},
                {fitting_result[5]},
                {max(fitting_result[6] - 1, -100.0f), min(fitting_result[6], 100.0f), 1},
                {fitting_result[7], fitting_result[7] + 1, 1}
            };
            #else
            std::vector<std::vector<float>> slices = {
                {fitting_result[0]},
                {fitting_result[1] - 0.1f, fitting_result[1] + 0.1f, 0.1},
                #ifdef FIVE_LEVELS
                    {std::max(fitting_result[2] - 20, 1.0f), std::min(fitting_result[2] + 20, 101.0f), 2},
                    {std::max(fitting_result[3] - 20, 1.0f), std::min(fitting_result[3] + 20, 101.0f), 2},
                    {std::max(fitting_result[4] - 20, 1.0f), std::min(fitting_result[4] + 20, 101.0f), 2},
                    {std::max(fitting_result[5] - 20, 1.0f), std::min(fitting_result[5] + 20, 101.0f), 2},
                    {std::max(fitting_result[6] - 20, 1.0f), std::min(fitting_result[6] + 20, 101.0f), 2},
                    {1},
                    {std::max(fitting_result[8] - 2, -100.0f), std::min(fitting_result[8] + 2, 100.0f), 0.1},
                    {std::max(fitting_result[9] - 0.01f, 0.0f), std::min(fitting_result[9] + 0.01f, 1.0f), 0.001}
                #else
                    {std::max(fitting_result[2] - 10, 1.0f), std::min(fitting_result[2] + 10, 101.0f), 1},
                    {std::max(fitting_result[3] - 10, 1.0f), std::min(fitting_result[3] + 10, 101.0f), 1},
                    {std::max(fitting_result[4] - 10, 1.0f), std::min(fitting_result[4] + 10, 101.0f), 1},
                    {1},
                    {std::max(fitting_result[6] - 1, -100.0f), std::min(fitting_result[6] + 1, 100.0f), 0.01},
                    {std::max(fitting_result[7] - 0.01f, 0.0f), std::min(fitting_result[7] + 0.01f, 1.0f), 0.0001}
                #endif
            };
            #endif
...
#ifdef DEBUG_SINGLE_TEST
    std::vector<int> slice_type = { PLAIN_TICKS, PLAIN_TICKS, PLAIN_TICKS, PLAIN_TICKS, PLAIN_TICKS, PLAIN_TICKS, PLAIN_TICKS, PLAIN_TICKS, PLAIN_TICKS, PLAIN_TICKS };
    std::unordered_map<std::string, float*> results;
    std::unordered_map<std::string, std::vector<std::vector<float>>> slices_by_subject;
    slices_by_subject["SUB0"] = std::vector<std::vector<float>>({{8}, {0.3}, {41}, {61}, {27}, {59}, {25}, {1}, {39}, {0.99}});
    fitAllSubjects(results, frame_blocks, slice_type, slices_by_subject);
#else

#ifdef FIVE_LEVELS
    std::vector<int> slice_type = { PLAIN_TICKS, SLICED_TICKS, SLICED_TICKS, SLICED_TICKS, SLICED_TICKS, SLICED_TICKS, SLICED_TICKS, PLAIN_TICKS, SLICED_TICKS, SLICED_TICKS };
#else
    std::vector<int> slice_type = { PLAIN_TICKS, SLICED_TICKS, SLICED_TICKS, SLICED_TICKS, SLICED_TICKS, PLAIN_TICKS, SLICED_TICKS, SLICED_TICKS };
#endif
    std::unordered_map<std::string, std::vector<std::vector<float>>> slices_by_subject = generateSliceData(&frame_blocks, nullptr);
    std::unordered_map<std::string, float*> results;
    fitAllSubjects(results, frame_blocks, slice_type, slices_by_subject);

    slices_by_subject = generateSliceData(&frame_blocks, &results);
    results.clear();
    fitAllSubjects(results, frame_blocks, slice_type, slices_by_subject);
#endif
