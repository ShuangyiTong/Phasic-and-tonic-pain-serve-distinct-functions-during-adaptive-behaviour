...
namespace model
{
                       ...
                       uint32_t get_item=frame_defs::OBJ_BOTTOM_TYPE)
    {
        ...
        for (int i = 0; i < frame_defs::MEM_SIZE; i++)
        {
            ...
            float x = frame_defs::INTEGER_WORD_TO_FLOAT(memory[i * 4 + 1]);
            float y = frame_defs::INTEGER_WORD_TO_FLOAT(memory[i * 4 + 2]);
            float z = frame_defs::INTEGER_WORD_TO_FLOAT(memory[i * 4 + 3]);
            float beta_dot_distance_times_C_v = free_parameter[VIGOUR_CONSTANT] * 
        ...
        #endif
            if (choice_value > current_optimal_Q) // logic short circuit, will check if bottom type first
            {
                ...
                if (pickup_choices_counter < num_choices)
                {
                    if (!skip_current_pickup)
                    {
                        if (current_frame[frame_defs::TIMESTAMP] > 0)
                        {
                            ...
                            frame_defs::INTEGER_WORD_TO_FLOAT(current_frame[frame_defs::AVG_SPEED]),
