...
LowNoPressureRAW = np.array([e.get_data(picks='Cz')[0] for e in [epochs["LowNoPressure"].average() for epochs in epochs_list]])
print(np.array([e.get_data(picks='Cz')[0] for e in [epochs["LowNoPressure"].average() for epochs in epochs_list]]).shape)
print(np.array(LowNoPressureRAW).shape)

HighNoPressure = mne.grand_average([epochs["HighNoPressure"].average() for epochs in epochs_list]).get_data(picks='Cz')[0]
HighNoPressureRAW = np.array([e.get_data(picks='Cz')[0] for e in [epochs["HighNoPressure"].average() for epochs in epochs_list]])

LowTonic = mne.grand_average([epochs["LowTonic"].average() for epochs in epochs_list]).get_data(picks='Cz')[0]
LowTonicRAW = np.array([e.get_data(picks='Cz')[0] for e in [epochs["LowTonic"].average() for epochs in epochs_list]])

HighTonic = mne.grand_average([epochs["HighTonic"].average() for epochs in epochs_list]).get_data(picks='Cz')[0]
HighTonicRAW = np.array([e.get_data(picks='Cz')[0] for e in [epochs["HighTonic"].average() for epochs in epochs_list]])
...
def erp_plot(ax, erp1, erp2, raw1, raw2, c1, c2, label1, label2, num):
    ax.plot(erp1 * 1e6, label=label1, color=c1, linewidth=3)
    ax.plot(erp2 * 1e6, label=label2, color=c2, linewidth=3)

    x = np.arange(len(erp1))
    diff_data = (raw2 - raw1) * 1e6
    mean_diff = np.mean(diff_data, axis=0)
    se_diff = stats.sem(diff_data, axis=0) # SE of the difference
    ci_95 = 1.96 * se_diff
    ax.fill_between(x, mean_diff - ci_95, mean_diff + ci_95, 
                    color='grey', alpha=0.2, edgecolor='none')
    ax.plot(x, mean_diff, label='Pairwise Diff', color='black', linewidth=3)
...
erp_plot(ax, LowNoPressure, HighNoPressure, LowNoPressureRAW, HighNoPressureRAW, sns.color_palette('Set1')[1], sns.color_palette('Set1')[0], 'Low phasic pain', 'High phasic pain', 'a')


ax = f.add_subplot(3, 2, 2)
ax.set_title('With tonic pain', fontsize=24)
erp_plot(ax, LowTonic, HighTonic, LowTonicRAW, HighTonicRAW, sns.color_palette('Set1')[1], sns.color_palette('Set1')[0], 'Low phasic pain', 'High phasic pain', 'b')

ax = f.add_subplot(3, 2, 3)

ax.set_title('Low phasic pain', fontsize=24)
erp_plot(ax, LowNoPressure, LowTonic, LowNoPressureRAW, LowTonicRAW, sns.color_palette('Set2')[0], sns.color_palette('Set2')[1], 'No tonic pain', 'With tonic pain', 'c')

ax = f.add_subplot(3, 2, 4)

ax.set_title('High phasic pain', fontsize=24)
erp_plot(ax, HighNoPressure, HighTonic, HighNoPressureRAW, HighTonicRAW, sns.color_palette('Set2')[0], sns.color_palette('Set2')[1], 'No tonic pain', 'With tonic pain', 'd')
