...
subjects = [k for i in range(5) for k, v in results.items()]
df = pd.DataFrame({"Ratings": dx, "PainFunc": dy, "Subject": subjects})
print(df)
...
m = lmer_test.lmer('PainFunc ~ Ratings + (1 + Ratings | Subject)', r_from_pd_df)
