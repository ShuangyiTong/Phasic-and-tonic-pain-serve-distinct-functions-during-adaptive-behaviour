...
from core.utils import NI_DEVICE_NAME, ARDUINO_DEVICE_NAME, EEG_NAME, LIVEAMP_DEVICE_NAME, LIVEAMP_DEVICE_NAME_BRAINVISION, nannorm, nan_square_sum
...
TRIM_LOCAL_DUMP_TO_FIRST_GREEN = False
def trim_dump_to_first_green(dump):
    if dump != dump:
        return math.nan
    new_dump = []
    has_first_green_past = False
    for trial in dump:
        if has_first_green_past:
            new_dump.append(trial)
        if has_first_green_past == False and trial['target'].endswith('G'):
            has_first_green_past = True
    if new_dump == []:
        return math.nan
    else:
        return new_dump

exp_data = make_experiment_data(exclusive_participants=[], exclude_participants=["SUB11"], exclude_device_data=[NI_DEVICE_NAME, ARDUINO_DEVICE_NAME, EEG_NAME, LIVEAMP_DEVICE_NAME, LIVEAMP_DEVICE_NAME_BRAINVISION], lazy_closure=True)
...
def subject_all_block_fitting(individual_data):
    ...
    if TRIM_LOCAL_DUMP_TO_FIRST_GREEN:
        in_block_simulation_dumps = [trim_dump_to_first_green(d) for d in in_block_simulation_dumps]
